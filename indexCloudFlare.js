...
export default {
  async fetch(request, env, ctx) {
    ...
    // Parse thread URLs: /:board/thread/:id or /:board/thread/:id/p:postId
    const threadMatch = pathname.match(/^\/([^\/]+)\/thread\/(\d+)(?:\/p(\d+))?$/);

    if (threadMatch) {
      const [, board, threadId, postId] = threadMatch;
      return handleThread(request, board, threadId, postId);
    }
...
async function handleThread(request, board, threadId, postId = null) {
  ...
  try {
    ...
    // Convert <br> tags to newlines and sanitize HTML
...
<body>
    ...
    console.error(err);
    return new Response('Error fetching thread', { status: 500 });
  }
}
